...
from src.module.diffusion.models.fp16_util import convert_module_to_f16, convert_module_to_f32
from src.module.diffusion.models.nn import (
    checkpoint,
    conv_nd,
    linear,
    avg_pool_nd,
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, x, context=None):
        ...
        x = self.norm(x)
        x = self.proj_in(x)
        x = rearrange(x, 'b c h w -> b (h w) c').contiguous() # UserWarning: Grad strides do not match bucket view strides.
...
class UNetModel(nn.Module):
    ...
    def forward_with_cfg(self, x, t, cfg_scale, context=None, y=None):
        """
        Forward pass of UNetModel, but also batches the unconditional forward pass for classifier-free guidance.
        """
        ...
        eps, rest = model_out[:, :2], model_out[:, 2:]
