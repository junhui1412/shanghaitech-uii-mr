...
class ResidualDiffusion(nn.Module):
    ...
    def model_predictions(self, model, x_input, x, t, x_input_condition=0, x_self_cond=None, clip_denoised=True):
    ...
    def p_mean_variance(self, model, x_input, x, t, x_input_condition=0, x_self_cond=None):
        preds = self.model_predictions(
            model, x_input, x, t, x_input_condition, x_self_cond)
    ...
    @torch.no_grad()
    def p_sample(self, model, x_input, x, t: int, x_input_condition=0, x_self_cond=None):
        b, *_, device = *x.shape, x.device
        batched_times = torch.full(
            (x.shape[0],), t, device=x.device, dtype=torch.long)
        model_mean, _, model_log_variance, x_start = self.p_mean_variance(
            model, x_input, x=x, t=batched_times, x_input_condition=x_input_condition, x_self_cond=x_self_cond)
        noise = torch.randn_like(x) if t > 0 else 0.  # no noise if t == 0
        pred_img = model_mean + (0.5 * model_log_variance).exp() * noise
        return pred_img, x_start

    @torch.no_grad()
    def p_sample_loop(self, model, x_input, shape, last=True):
        if self.input_condition:
        ...
        for t in tqdm(reversed(range(0, self.num_timesteps)), desc='sampling loop time step', total=self.num_timesteps):
            ...
            img, x_start = self.p_sample(
                model, x_input, img, t, x_input_condition, self_cond)
    ...
    @torch.no_grad()
    def ddim_sample(self, model, x_input, shape, last=True):
        if self.input_condition:
            x_input_condition = x_input[1]
        else:
            x_input_condition = 0
        ...
        batch, device, total_timesteps, sampling_timesteps, eta, objective = shape[
            0], x_input.device, self.num_timesteps, self.sampling_timesteps, self.ddim_sampling_eta, self.objective
        ...
        for time, time_next in tqdm(time_pairs, desc='sampling loop time step'):
            ...
            self_cond = x_start if self.self_condition else None
            preds = self.model_predictions(
                model, x_input, img, time_cond, x_input_condition, self_cond)
    ...
    @torch.no_grad()
    def sample(self, model, x_input, batch_size=16, last=True):
        image_size, channels = self.image_size, model.channels
        ...
        return sample_fn(model, x_input, size, last=last)
    ...
    def p_losses(self, model, imgs, t, noise=None):
        ...
        if self.self_condition and torch.rand(1).item() < 0.5:
            with torch.no_grad():
                x_self_cond = self.model_predictions(
                    model, x_input, x, t, x_input_condition if self.input_condition else 0).pred_x_start
                x_self_cond.detach_()
        ...
        model_out = model(x_in, t, x_self_cond)
    ...
    def training_losses(self, model, img, t, *args, **kwargs):
