...
class NerfEmbedder(nn.Module):
    ...
    def forward(self, inputs):
        ...
        dct = dct.repeat(B, 1, 1) # [B, 16**2, max_freqs**2]
...
class NerfBlock(nn.Module):
    ...
    def forward(self, x, s):
        ...
        mlp_params1 = self.param_generator1(s) # [b * l, 32768]
        fc1_param1, fc2_param1 = mlp_params1.chunk(2, dim=-1) # [b * l, 16384]
...
class PixNerDiT(ModelMixin, ConfigMixin):
    ...
    def forward(self, x, timesteps, context=None, y=None, s=None, mask=None):
        t = timesteps
        ...
        # x = x.transpose(1, 2)
        x = rearrange(x, 'b l (c p1 p2) -> (b l) (p1 p2) (c)', p1=self.patch_size, p2=self.patch_size)
        s = s.view(batch_size*length, self.hidden_size)
        x = self.x_embedder(x) # [b * l, p1 * p2, 64]
