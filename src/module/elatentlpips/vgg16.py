...
class CalibratedLatentVGG16BN(torch.nn.Module):
    def __init__(self, num_latent_channels, encoder, pretrained=False, requires_grad=True):
        ...
        if pretrained:
            ...
            if not os.path.exists('./model_weight/latent_lpips/ckpt'):
                os.makedirs('./model_weight/latent_lpips/ckpt', exist_ok=True)
            
            if not os.path.exists(f'./model_weight/latent_lpips/ckpt/{encoder}_latent_vgg16.pth.tar'):
                torch.hub.download_url_to_file(url_path, f'./model_weight/latent_lpips/ckpt/{encoder}_latent_vgg16.pth.tar')
            ckpt = torch.load(f'./model_weight/latent_lpips/ckpt/{encoder}_latent_vgg16.pth.tar', map_location=lambda storage, loc: storage)
