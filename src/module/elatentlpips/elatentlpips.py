...
from src.module import elatentlpips
from .vgg16 import CalibratedLatentVGG16BN
from .ada_aug import AdaAugment
        ...
        if(elatentlpips):
            ...
            if(pretrained):
                if self.encoder in ["sd15", "sd21", "sdxl", "sd3", "flux"]:
                    ...
                    if not os.path.exists(f"./model_weight/latent_lpips/ckpt/{self.encoder}_latest_{self.pnet_type}_tuned.pth"):
                        torch.hub.download_url_to_file(url_path, f"./ckpt/{self.encoder}_latest_{self.pnet_type}_tuned.pth")
                    ckpt = torch.load(f"./model_weight/latent_lpips/ckpt/{self.encoder}_latest_{self.pnet_type}_tuned.pth", map_location=lambda storage, loc: storage)
                else:
