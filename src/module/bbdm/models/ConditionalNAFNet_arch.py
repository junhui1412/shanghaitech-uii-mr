...
class ConditionalNAFNet(nn.Module):
    ...
    def __init__(self, img_channel=3, width=16, middle_blk_num=1, enc_blk_nums=[], dec_blk_nums=[], upscale=1, condition_key="nocond"):
        super().__init__()
    ...
    def forward(self, inp, context, timesteps):
        inp_res = inp.clone()

        if isinstance(timesteps, int) or isinstance(timesteps, float):
            timesteps = torch.tensor([timesteps]).to(inp.device)
