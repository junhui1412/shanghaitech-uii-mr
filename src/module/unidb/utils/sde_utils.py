...
from src.module.unidb.modules.loss import MatchingLoss
...
class SDE(abc.ABC):
    ...
    def reverse_sde(self, model, xt, T=-1):
        T = self.T if T < 0 else T
        x = xt.clone()
        for t in tqdm(reversed(range(1, T + 1))):
            score = self.score_fn(model, x, t)
...
class UniDB(SDE):
    ...
    def __init__(self, lambda_square, gamma, T=100, schedule='cosine', eps=0.01, loss_type="l1", is_weighted=False, device=None):
        super().__init__(T, device)
        self.lambda_square = lambda_square / 255 if lambda_square >= 1 else lambda_square
        self.gamma = gamma
        self.loss_fn = MatchingLoss(loss_type, is_weighted).to(device)
    ...
    def score_fn(self, model, x, t, **kwargs):
        # need to pre-set mu and score_model
        return model(x, self.mu, t, **kwargs)

    def noise_fn(self, model, x, t, **kwargs):
        # need to pre-set mu and score_model
        return model(x, self.mu, t, **kwargs)
    ...
    def reverse_sde(self, model, xt, T=-1, save_states=False, save_dir='sde_state', **kwargs):
        T = self.T if T < 0 else T
        self.set_mu(xt)
        x = xt.clone()
        for t in tqdm(reversed(range(1, T + 1)), total=T):
            noise = model(x, self.mu, t, **kwargs)
    ...
    def reverse_mean_ode(self, model, xt, T=-1, save_states=False, save_dir='ode_state', **kwargs):
        T = self.T if T < 0 else T
        self.set_mu(xt)
        x = xt.clone()
        for t in tqdm(reversed(range(1, T + 1)), total=T):
    ...
    def training_losses(self, model, x0, mu):
        terms = {}
        timesteps, states = self.generate_random_states(x0=x0, mu=mu)
        timesteps = timesteps.to(self.device)
        # Get noise and score
        noise = self.noise_fn(model, states, timesteps.squeeze())
        score = self.get_score_from_noise(noise, timesteps)

        # Learning the maximum likelihood objective for state x_{t-1}
        xt_1_expection = self.reverse_sde_step_mean(states, score, timesteps)
        xt_1_optimum = self.reverse_optimum_step(states, x0, timesteps)
