...
from diffusers import EDMEulerScheduler, EDMDPMSolverMultistepScheduler
...
def main():
    ...
    model = UNetModel(
        ...
        in_channels=2,
        model_channels=args.model_channels, # 256 # 64
        out_channels=1,
        num_res_blocks=2,
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(dataloader):
            ...
            with accelerator.accumulate(model):
                ...
                loss_dict = diffusion.training_losses(model, output_images, context=input_images)
                ...
                if (global_step == 1 or (global_step % args.sample_every == 0 and global_step > 0)):
                    with torch.no_grad():
                        ...
                        for val_step, val_batch in enumerate(val_dataloader):
                            ...
                            samples = diffusion.edm_sampler(model, torch.randn_like(val_input_images), context=val_input_images, num_steps=args.sample_steps, dtype=torch.float32)
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        # default=None,
        default="cfg/train_configs/train_edm.yaml",
        # required=True,
        help="path to config",
    )
